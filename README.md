...
## сылка на "Кошачий пинтерест"
..

[Посмотреть сайт](https://love-cats-nu.vercel.app/)

## Проблемы с подключением к API

К сожалению, я столкнулся с проблемами при подключении к API через проксирование. Сначала всё работало корректно, но через пару часов подключение перестало функционировать. В результате, для доступа к API теперь требуется использовать VPN.
